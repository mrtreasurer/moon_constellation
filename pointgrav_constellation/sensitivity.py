...
inc_range = np.arange(0, 91, 1)*np.pi/180

inc_results = np.zeros((inc_range.shape[0], 3))
for j, inc in enumerate(inc_range):
    ...
    inc_results[j] = [inc, np.mean(charge), np.min(charge)]

np.savetxt("data/inc_sensitivity.csv", inc_results, delimiter=",")
print("\rInclination analysis finished")

sma_range = np.arange(cte.r_m, cte.max_sat_range + cte.r_m, 1e4)

sma_results = np.zeros((sma_range.shape[0], 3))
for i, sma in enumerate(sma_range):
    ...
    sma_results[i] = [sma, np.mean(charge), np.min(charge)]
